                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse ($exams as $exam)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <!-- <td class="px-6 py-4">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $exam->status == 'active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                                {{ ucfirst($exam->status) }}
                                            </span>
                                        </td> -->
                                    </tr>
                    ...
                    <form action="{{ route('dashboard') }}" method="GET">
                        <div class="flex flex-col sm:flex-row items-center gap-4">
                            ...
                            <div class="sm:ml-auto">
                                 <a href="{{ route('exam-results.export-pdf', request()->only(['due_date'])) }}" class="inline-flex items-center w-full sm:w-auto justify-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 transition">Export PDF</a>
                            </div>
                        </div>
                    </form>
                </div>
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-4">
                                                @if($result->face_image_path)
                                                    <button @click="showModal = true; modalImage = '{{ asset('storage/' . $result->face_image_path) }}'">
                                                        <img src="{{ asset('storage/' . $result->face_image_path) }}" alt="Foto" class="w-12 h-12 rounded-lg object-cover cursor-pointer hover:opacity-80 transition-opacity">
                                                    </button>
                                                @else
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                            Tidak ada hasil ulangan yang cocok dengan filter Anda.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
